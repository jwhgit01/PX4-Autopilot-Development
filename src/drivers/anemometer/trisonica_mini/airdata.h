...
class Airdata : public ModuleBase<Airdata>, public ModuleParams {
	...
	private:
		...
		int read_airdaq(int port_fd, orb_advert_t adu_pub, struct airdata_s adu) {
			...
			adu.alpha_deg = (float) -alpha_deg;	// to account for boom orientation
			adu.beta_deg = (float) (atan(tan(betaf_deg*convFact)*cos(alpha_deg*convFact))/convFact); // +beta = wind in the right ear
